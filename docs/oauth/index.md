...
咸鱼云网盘同时提供两套可并行使用的开放平台鉴权能力：

1. **标准 OIDC / OAuth 2.1 端点**：供标准客户端直接接入
2. **原有开放平台 OAuth 链路**：供现有客户端继续使用
...
旧链路仍采用授权码模式，支持第三方应用获取用户授权后访问咸鱼云网盘开放接口。整个流程分为以下几个步骤：
