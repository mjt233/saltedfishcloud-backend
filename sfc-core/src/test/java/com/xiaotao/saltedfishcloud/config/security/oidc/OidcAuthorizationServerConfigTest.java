 ...
 *   <li>验证 {@link OidcJwkService#loadOrCreate(OidcServerProperty)} 生成正确 keyId 的 JWKSet</li>
...
class OidcAuthorizationServerConfigTest {
    ...
    static class TestConfig {
        ...
        /**
         * 提供测试用的注册客户端仓库（空库），满足授权服务器初始化所需的最小依赖。
         */
        @Bean
     ...
     * 生成的 Bean 包含正确的 issuer、授权端点和 JWK Set 端点。
     * <p>
     * 纯单元验证：直接实例化配置类，无需 Spring 上下文注入。
     * </p>
     */
    @Test
    ...
    /**
     * 验证 {@link OidcJwkService#loadOrCreate(OidcServerProperty)} 生成的 JWKSet
    ...
    void jwkServiceShouldGenerateJwkSetWithConfiguredKeyId() {
        ...
        JWKSet jwkSet = service.loadOrCreate(prop);
