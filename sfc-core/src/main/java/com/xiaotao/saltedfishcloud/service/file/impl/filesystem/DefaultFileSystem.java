...
public class DefaultFileSystem implements DiskFileSystem, FeatureProvider, InitializingBean {
    ...
    public void move(long uid, String source, String target, String name, boolean overwrite) throws IOException {
        DistributedLock lock = lockFactory.getLock(getStoreLockKey(uid, target, name));
        try {
            lock.lock();
    ...
    public void moveToSaveFile(long uid, Path nativeFilePath, String path, FileInfo fileInfo) throws IOException {
        DistributedLock lock = lockFactory.getLock(getStoreLockKey(uid, path, fileInfo.getName()));
        try {
            lock.lock();
            fileInfo.setUid(uid);
    ...
    public long deleteFile(long uid, String path, List<String> name) throws IOException {
        ArrayList<DistributedLock> locks = new ArrayList<>();
        for (String s : name) {
            DistributedLock lock = lockFactory.getLock(getStoreLockKey(uid, path, s));
            lock.lock();
            locks.add(lock);
        }
        try {
    ...
    public void rename(long uid, String path, String name, String newName) throws IOException {
        DistributedLock lock1 = lockFactory.getLock(getStoreLockKey(uid, path, name));
        DistributedLock lock2 = lockFactory.getLock(getStoreLockKey(uid, path, newName));
        lock1.lock();
        lock2.lock();
        try {
