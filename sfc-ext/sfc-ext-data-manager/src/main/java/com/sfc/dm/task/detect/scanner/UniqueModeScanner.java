...
public class UniqueModeScanner extends AbstractInvalidDataScanner {
    ...
    public void scan(Consumer<InvalidDataRecord> callback) {
        ...
        try {
            scanUniqueStorage(storage, storeRoot, validMd5Set, callback);
        } catch (IOException e) {
        ...
        log("扫描失效文件记录...");
        scanInvalidFileRecords(storage, storeRoot, callback);
     ...
     * @param callback 发现失效数据记录时的回调
     */
    private void scanFileRecordsByUid(Long uid, Storage storage, String repoRoot, Consumer<InvalidDataRecord> callback) {
     ...
     * @param callback    发现失效数据记录时的回调
     */
    private void scanUniqueStorage(Storage storage, String path, Set<String> validMd5Set,
        ...
        for (FileInfo file : files) {
            ...
            if (file.isDir()) {
                scanUniqueStorage(storage, fullPath, validMd5Set, callback);
            } else {
                // 无拓展名的文件即为物理存储文件
