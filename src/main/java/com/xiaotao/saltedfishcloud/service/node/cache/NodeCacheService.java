...
import static com.xiaotao.saltedfishcloud.service.node.cache.NodeCacheKeyGenerator.*;
...
public class NodeCacheService {
    ...
    private void deleteNodeCache(int uid, Collection<String> nodeIds, boolean queryParent) throws JsonProcessingException {
        ...
        for (String nid : nodeIds) {
            // nid的所有子节点的缓存key集合
            Set<String> subNodeKeys = redisDao.scanKeys(generatePnidKey(uid, nid, "*"));
            ...
            for (String subNode : subNodeKeys) {
                ...
                subids.add(n.getId());
                redisTemplate.delete(generatePnidKey(uid, nid, n.getName()));
            }
            // 继续对查找出的子节点进行删除操作
            ...
            if (queryParent) {
                ...
                redisTemplate.delete(generatePnidKey(uid, nodeInfo.getParent(), nodeInfo.getName()));
            }
